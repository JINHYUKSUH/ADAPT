...
from test_util.detection_util import set_ood_loader_ImageNet, obtain_feature_from_loader, set_ood_loader_small, get_and_print_results
from test_util.util import set_loader_ImageNet, set_loader_small
from test_util.display_results import  plot_distribution, print_measures, save_as_dataframe
...
def process_args():
    ...
    parser.add_argument('--loss', default = 'ADAPT', type=str, help='loss of experiment')
    ...
    parser.add_argument('--save_path', type=str, default='', help='trained model')
...
def set_up(args): 
    ...
    net = get_model(args, args.n_cls, load_ckpt=True)
    net.to(device)
    net.eval()
...
def main(args):
    ...
    if args.in_dataset == 'ImageNet-100':
        out_datasets = ['SUN', 'places365', 'texture', 'iNaturalist']
    else: 
        out_datasets = [ 'SVHN', 'iSUN', 'LSUN_resize', 'Imagenet_resize', 'texture' ]
