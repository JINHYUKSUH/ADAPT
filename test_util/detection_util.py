...
#                                          std=[x/255.0 for x in [63.0, 62.1, 66.7]])
    ...
    if out_dataset == 'SVHN':
        testsetout = SVHN(root=os.path.join(root, 'svhn'), split='test',
            ...
            testsetout = torchvision.datasets.ImageFolder(root= os.path.join(root, 'texture'),
            transform=transforms.Compose([transforms.Resize(img_size), 
            transforms.CenterCrop(img_size), transforms.ToTensor(),normalize]))
    elif out_dataset == 'LSUN_resize':
            testsetout = torchvision.datasets.ImageFolder(root= os.path.join(root, 'LSUN_resize'),
            transform=transforms.Compose([transforms.Resize(img_size), 
            transforms.CenterCrop(img_size), transforms.ToTensor(),normalize]))
    elif out_dataset == 'iSUN':
            testsetout = torchvision.datasets.ImageFolder(root= os.path.join(root, 'iSUN'),
            transform=transforms.Compose([transforms.Resize(img_size), 
            transforms.CenterCrop(img_size), transforms.ToTensor(),normalize]))
    elif out_dataset == 'Imagenet_resize':
            testsetout = torchvision.datasets.ImageFolder(root= os.path.join(root, 'imagenet_resize'),
            transform=transforms.Compose([transforms.Resize(img_size), 
            transforms.CenterCrop(img_size), transforms.ToTensor(),normalize]))
    else:
